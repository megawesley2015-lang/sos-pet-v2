...
import { deletePet, marcarComoResolvido } from "@/services/pets.service";
...
export default function MeusPetsPage() {
  ...
  useEffect(() => {
    async function checkAuthAndFetchPets() {
      try {
  ...
  const handleResolver = async (petId) => {
    ...
    setActionLoading(petId);
    try {
      await marcarComoResolvido(petId);
      setPets(pets.filter((p) => p.id !== petId));
    } catch (err) {
  ...
  if (!user) {
    return null; // Redirect acontece no useEffect
  }
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-500">
              Gerencie seus registros de pets perdidos, encontrados ou para adoção
            </p>
          </div>
